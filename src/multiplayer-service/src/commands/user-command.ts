...
export class UserJoinCommand extends Command<PokieRoom, UserJoinCommandPayload> {
	execute({ sessionId, username, position, address, skin }: UserJoinCommandPayload) {
		this.state.createUser(sessionId, username, address, position, skin)
	}
}

export class UserMoveCommand extends Command<PokieRoom, UserMoveCommandPayload> {
...
export class UserChangeAnimationCommand extends Command<PokieRoom, UserChangeAnimationCommandPayload> {
	execute({ sessionId, animation }: UserChangeAnimationCommandPayload) {
		this.state.players.get(sessionId).setAnimation(animation)
	}
}

export class UserLeaveCommand extends Command<PokieRoom, UserLeaveCommandPayload> {
